...
-keep class io.neuralheads.kmpshortcuts.** { *; }

# Keep the @Volatile singleton field for correct cross-thread visibility
-keepclassmembers class io.neuralheads.kmpshortcuts.KMPShortcuts {
    volatile <fields>;
}

# Keep MaterialSymbolMapper lookup table (referenced by string key)
-keepclassmembers class io.neuralheads.kmpshortcuts.ShortcutIconResolver {
    *;
}
-keepclassmembers class io.neuralheads.kmpshortcuts.MaterialSymbolMapper {
    *;
}
