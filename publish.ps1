# ==============================================================================
# KMPShortcuts -- One-click Maven Central publisher
# Usage: .\publish.ps1
#
# Reads credentials from: C:\projects\plugins\credentials.properties
# ==============================================================================
...
# -- 3. Clean staging dir -----------------------------------------------------
$stagingDir = "C:\kmpshortcuts-release"
...
# We need a local Maven repo to publish to (avoids vanniktech Windows URI bug)
$env:ORG_GRADLE_PROJECT_mavenCentralUsername = $sonatypeUser
$env:ORG_GRADLE_PROJECT_mavenCentralPassword = $sonatypePass
...
# Check if LocalRelease repo is configured; if not, publish to MavenLocal first
$hasLocalRelease = Select-String -Path "$PSScriptRoot\build.gradle.kts" -Pattern "LocalRelease" -Quiet 2>$null
if (-not $hasLocalRelease) {
    # Fallback: publish to a local directory via Gradle property
    $gradleArgs = @(
        "publishToMavenLocal",
        "--no-daemon", "--no-configuration-cache",
        "-x", "test", "-x", "lint"
    )
    $stagingDir = "$env:USERPROFILE\.m2\repository\com\neuralheads"
}

& "$PSScriptRoot\gradlew.bat" @gradleArgs
if ($LASTEXITCODE -ne 0) { Write-Error "Gradle build failed"; exit 1 }

$ascCount = (Get-ChildItem $stagingDir -Recurse -Filter "*.asc" -ErrorAction SilentlyContinue | Measure-Object).Count
Write-Host "OK  Build SUCCESS -- $ascCount signed artifacts"

# -- 5. Zip the bundle ---------------------------------------------------------
Add-Type -AssemblyName System.IO.Compression.FileSystem
$bundlePath = "$PSScriptRoot\kmpshortcuts-bundle.zip"
...
$deployName = "kmpshortcuts-$version"
$amp        = [char]38
$uploadUrl  = "https://central.sonatype.com/api/v1/publisher/upload?publishingType=AUTOMATIC$($amp)name=$deployName"
...
for ($i = 1; $i -le $maxWait; $i++) {
    ...
    if ($state -eq "PUBLISHED") {
        Write-Host ""
        Write-Host "SUCCESS! com.neuralheads:kmpshortcuts:$version is LIVE on Maven Central"
        Write-Host "  https://central.sonatype.com/artifact/com.neuralheads/kmpshortcuts/$version"
        exit 0
    }
    if ($state -eq "FAILED") {
